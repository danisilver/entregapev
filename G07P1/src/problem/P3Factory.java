...
import mapper.F3Mapper;
import utils.Utils;
import view.View;

public class P3Factory implements ProblemFactory{
	...
	public Cromosoma createCromosoma(String tipo, HashMap<String,Object> props) {
		if(tipo.equalsIgnoreCase("Binario")) {
			...
			Cromosoma2DF3 crom = new Cromosoma2DF3();
	...
	public View createView(HashMap<String,Object> props) {
		...
		surface = Surface.shape(new F3Mapper(), 
				new Range(-10f, 10f), 
				new Range(-10f, 10f), 
				steps, 
				new ColorMapRainbow(),
		...
		MapperContourPictureGenerator contour = new MapperContourPictureGenerator(
				new F3Mapper(), new Range(-10f, 10f), new Range(-10f, 10f));
